...
function shouldUseSearch(native){
...
function mapNormSpanToOrig(label, normStart, normLen){
  const chars = Array.from(label);
  let acc = 0, start = 0, end = chars.length;
  for (let i=0;i<chars.length;i++){
    const n = normalizeText(chars[i]);
    if (acc <= normStart && (acc + n.length) > normStart) start = i;
    if (acc < normStart + normLen && (acc + n.length) >= normStart + normLen){ end = i+1; break; }
    acc += n.length;
  }
...
function highlightLabel(label, q){
  ...
  const idx = normLabel.indexOf(normQ);
  if (idx < 0) return escapeHTML(label);
  const [start,end,chars] = mapNormSpanToOrig(label, idx, normQ.length);
  const before = escapeHTML(chars.slice(0,start).join(''));
  const middle = escapeHTML(chars.slice(start,end).join(''));
  const after  = escapeHTML(chars.slice(end).join(''));
  return `${before}<mark class="mrr-hl">${middle}</mark>${after}`;
}

/* ---------- Label fitting ---------- */
function measureTextWidth(containerEl, text) {
  ...
  ['fontFamily','fontSize','fontWeight','fontStyle','letterSpacing','textTransform',
   'textIndent','textRendering','textDecoration','wordSpacing','lineHeight']
   .forEach(p => probe.style[p] = cs[p]);
  document.body.appendChild(probe);
  const w = probe.getBoundingClientRect().width;
  probe.remove();
  return w;
}
function getAvailableLabelWidth(dd, selectedEl) {
  const input = dd.querySelector('.mrr-input');
  if (!input) {
    const w = selectedEl.getBoundingClientRect().width || 0;
    return w;
  }
  ...
  const icon = dd.querySelector('.icon-down');
  if (icon) {
    const iconW = icon.getBoundingClientRect().width || 0;
    const ics = getComputedStyle(icon);
    available -= iconW + ((parseFloat(ics.marginLeft) || 0) + (parseFloat(ics.marginRight) || 0));
  }
  return Math.max(0, available - 8);
}
function buildFittedText(selectedEl, labels, dd) {
  const available = getAvailableLabelWidth(dd, selectedEl);
  if (!Number.isFinite(available) || available <= 0) {
    return { text: labels.join(', '), hidden: Math.max(0, labels.length - 1) };
  }
  const shown = []; let text = '';
  for (let i = 0; i < labels.length; i++) {
    const candidate = shown.length ? `${text}, ${labels[i]}` : labels[i];
    const remaining = labels.length - (i + 1);
    const suffix = remaining > 0 ? ` (+${remaining})` : '';
    if (measureTextWidth(selectedEl, candidate + suffix) <= available) {
      shown.push(labels[i]); text = candidate;
    } else break;
  }
  const hidden = labels.length - shown.length;
  if (hidden > 0) text = (shown.join(', ') || '') + (shown.length ? ` (+${hidden})` : `(+${hidden})`);
  return { text, hidden };
}
...
function getItemValue(item){
  ...
  if (item.hasAttribute('value')) {
    const v = item.getAttribute('value'); return v.trim()===''? null : v;
  }
  const span = item.querySelector('span');
  return getText(span) || getText(item) || null;
}
function labelsFromItems(items){
  return items.map(i => getText(i.querySelector('span')) || getText(i)).filter(Boolean);
}
...
function openDropdown(dd){
  ...
  content.addEventListener('animationend', function handler(e){
    ...
    dd.classList.remove('is-animating','is-opening');
...
function closeDropdown(dd){
  ...
  content.addEventListener('animationend', function handler(e){
    if (e.target !== content) return;
    content.removeEventListener('animationend', handler);
    dd.classList.remove('is-animating','is-closing','is-open');
...
/* ---------- Selection / visual ---------- */
function computeSelectionData(dd){
  const items  = getSelectedItems(dd);
  const labels = labelsFromItems(items);
  const values = items.map(getItemValue).filter(v => v != null);
  return { items, labels, values };
}
function updateSelectedVisual(dd){
  const sel = dd.querySelector('.valor-selected'); if (!sel) return;
  const original = sel.dataset.originalPlaceholder || sel.textContent || 'Selecione';
  const { labels, values } = computeSelectionData(dd);

  if (!isMultiple(dd)) {
    if (labels.length){ sel.classList.remove('placeholder'); sel.textContent = labels[0]; }
    else { sel.classList.add('placeholder'); sel.textContent = original; }
  } else {
    if (!labels.length){ sel.classList.add('placeholder'); sel.textContent = original; }
    else { sel.classList.remove('placeholder'); sel.textContent = buildFittedText(sel, labels, dd).text; }
  }
...
function setActiveByValues(dd, values){
  const S = new Set(values.map(v => String(v).trim()).filter(Boolean));
  getAllItems(dd).forEach(item => {
    const val   = item.getAttribute('value');
    const label = getText(item.querySelector('span')) || getText(item);
...
/* ---------- Events ---------- */
function dispatchChange(dd){
  const { labels, values } = computeSelectionData(dd);
  dd.dispatchEvent(new CustomEvent('mrr:change', {
    bubbles: true,
    detail: {
      dropdown: dd,
      dataId:   dd.getAttribute('data-id') || null,
      multiple: isMultiple(dd),
      labels, values
    }
  }));
}
function onItemClick(dd, item){
  if (item.classList.contains('is-disabled')) return;
  if (isMultiple(dd)) {
    item.classList.toggle('ativo');
    updateSelectedVisual(dd);
  } else {
    getAllItems(dd).forEach(i => i.classList.remove('ativo'));
...
function handleDocClick(e){
  $$('.mrr-dropdown.is-open').forEach(dd => {
    const inp = dd.querySelector('.mrr-input');
    const cnt = dd.querySelector('.mrr-content-dropdown');
    if (!(inp && inp.contains(e.target)) && !(cnt && cnt.contains(e.target))) {
      closeDropdown(dd);
    }
  });
}
...
function bindSearch(dd){
  const input = dd.querySelector('.mrr-search-input');
  ...
  function applyFilter(){
    ...
    items.forEach(it => {
      ...
      const norm = it.dataset.norm || normalizeText(base);
  ...
  const content = dd.querySelector('.mrr-content-dropdown');
  if (content){
    content.addEventListener('animationend', () => {
      if (!isOpen(dd) && input.value) { input.value=''; applyFilter(); }
    });
  }
}

/* ---------- <select> <-> dropdown sync (anti-reentrância) ---------- */
function syncSelectFromDropdown(dd, values){
  const native = dd.__nativeSelect; if (!native) return;

  native.__mrrSyncing = true;

  const S = new Set(values.map(String));
  Array.from(native.options).forEach(opt => {
    opt.selected = S.has(opt.value) || (S.size===0 && opt.value==='' && !native.multiple);
  });

  native.dispatchEvent(new Event('change', { bubbles:true })); // listener ignora via __mrrSyncing
}
function syncDropdownFromSelect(native){
  const dd = native.__mrrDropdown; if (!dd) return;

  dd.__fromNative = true;
  const values = Array.from(native.selectedOptions).map(o => o.value);
  setActiveByValues(dd, values);
  updateSelectedVisual(dd);
  if (native.disabled) dd.classList.add('is-disabled'); else dd.classList.remove('is-disabled');
  dd.__fromNative = false;
}

/* ---------- Build list helpers (items + groups) ---------- */
  ...
  children.forEach(node => {
    if (node.tagName === 'OPTGROUP') {
      ...
      Array.from(node.children).forEach(opt => {
        ...
        if (node.disabled) item.classList.add('is-disabled');
        gitems.appendChild(item);
      });

      g.appendChild(glabel);
      g.appendChild(gitems);
      list.appendChild(g);
    } else if (node.tagName === 'OPTION') {
      list.appendChild(createItem(node)); // opção fora de grupo
    }
  });
...
/* ---------- Build dropdown markup from <select> ---------- */
function buildDropdownFromSelect(native){
  ...
  content.className = 'mrr-content-dropdown hidden';

  // search (opcional)
  if (shouldUseSearch(native)) {
    const s = document.createElement('div');
    s.className = 'mrr-search';
    const si = document.createElement('input');
    si.type = 'text';
    si.name = native.getAttribute('data-search-name') || `mrr-search-${dd.dataset.id || 'anon'}`;
    si.autocomplete = 'off';
    si.className = 'mrr-search-input';
    si.placeholder = native.getAttribute('data-search-placeholder') || 'Pesquisar...';
  ...
  // link bidirecional
  dd.__nativeSelect = native; native.__mrrDropdown = dd;
...
/* ---------- Public API ---------- */
window.mrr_selected_dropdown = function(id, rawValue){
  ...
  dd.setAttribute('data-valor', values.join(',')); setActiveByValues(dd, values); updateSelectedVisual(dd);
};
window.mrr_render_itens = function(id, items){
  // aceita flat [{value,label}] ou grupos [{ label, items:[...] }]
  const dd = document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (!dd) return;
  const content = dd.querySelector('.mrr-content-dropdown'); if (!content) return;
...
window.mrr_get_selected_values = function(id){
  const dd = document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (!dd) return [];
  return getSelectedItems(dd).map(getItemValue).filter(v=>v!=null);
};
window.mrr_get_selected_labels = function(id){
  const dd = document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (!dd) return [];
  return getSelectedItems(dd).map(i => getText(i.querySelector('span')) || getText(i)).filter(Boolean);
};
window.mrr_open_dropdown   = function(id){ const dd=document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (dd && !isOpen(dd)) openDropdown(dd); };
window.mrr_close_dropdown  = function(id){ const dd=document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (dd &&  isOpen(dd)) closeDropdown(dd); };
window.mrr_toggle_dropdown = function(id){ const dd=document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (dd) toggleDropdown(dd); };
window.mrr_clear_dropdown  = function(id){
  const dd=document.querySelector(`.mrr-dropdown[data-id="${id}"]`); if (!dd) return;
  dd.setAttribute('data-valor',''); getAllItems(dd).forEach(i=>i.classList.remove('ativo')); updateSelectedVisual(dd);
};

/* ---------- Bind + Boot ---------- */
function bindOneDropdown(dd){
  if (dd.dataset.mrrInit === '1') return;
  dd.dataset.mrrInit = '1';

  const selLbl = dd.querySelector('.valor-selected');
  if (selLbl) selLbl.dataset.originalPlaceholder = selLbl.textContent;
  ...
  const raw = (dd.getAttribute('data-valor') || '').trim();
...
function initMrrDropdownsFromSelects(){
  document.addEventListener('dragstart', e => {
    const t = e.target; if (t && (t.tagName === 'A' || t.tagName === 'IMG')) preventDragStart(e);
  }, { passive:false });

  $$('select.mrr-dropdown').forEach(native => {
    ...
    native.addEventListener('change', () => {
      if (native.__mrrSyncing) {
        native.__mrrSyncing = false; // consumimos o change "nosso"
        return;
      }
      syncDropdownFromSelect(native); // change "real"
    });

    syncDropdownFromSelect(native); // estado inicial
  });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initMrrDropdownsFromSelects, { once:true });
} else {
